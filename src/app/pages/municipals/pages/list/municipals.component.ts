...
import * as MunicipalitiesActions from "../../store/actions/municipals.actions";
...
@Component({
  ...
  template: `
    <div class="grid gap-3">
      ...
      <div *ngIf="municipalitiesPaginate$ | async as municipalitiesPaginate else skeleton">
        <app-table
...
export default class MunicipalsComponent implements AfterViewInit {
  ...
  municipalitiesPaginate$ = this.store.select(getMunicipalsPaginate);
  ...
  openDialog(municipality: PartialMunicipal) {
    const dialogRef: any = this.dialog.open(ModalComponent, {
      backdropClass: "blur-filter",
      data: <ModalDialogData>{
        title: "Conferma rimozione",
        content: `
        Si sta eliminando il comune di ${ municipality.city }.
        <br>
    ...
    dialogRef.afterClosed().subscribe((result: any) => {
      ...
      this.deleteMunicipal(municipality);
    });
  }
  ...
  private deleteMunicipal(row: PartialMunicipal) {
    this.store.dispatch(MunicipalitiesActions.deleteMunicipal({ id: row.id! }));
  }
